...
class FamilyExpenseInputScreen extends StatefulWidget {
  const FamilyExpenseInputScreen({Key? key}) : super(key: key);

  @override
  State<FamilyExpenseInputScreen> createState() =>
      _FamilyExpenseInputScreenState();
}

class _FamilyExpenseInputScreenState extends State<FamilyExpenseInputScreen> {
  final FamilyExpenseController expenseController =
      Get.put(FamilyExpenseController());
  ...
  Widget build(BuildContext context) {
    if (expenseController.e_Category.isEmpty) {
      expenseController.updateFamilyExpenseCategoryList();
    }
    return Scaffold(
      appBar: AppBar(
          ...
          IconButton(
              onPressed: () {
                Navigator.of(context).pushReplacement(MaterialPageRoute(
                    builder: (context) => FamilyExpenseInputScreen()));
              },
                              ...
                              GestureDetector(
                                onTap: () {
                                  showDialog(
                                    context: context,
                                    builder: (context) {
                                      return CategoryManagePopup("familyExpense");
                                    },
                                  );
                                  setState(() {
                                    controller.refresh();
                        ...
                        onPressed: () {
                          if (_formKey.currentState!.validate()) {
                            if (_Expanse_category == null) {
                              showTopTitleSnackBar(context, Icons.error,
                            ...
                            if (_Expanse_category != null &&
                                _expanseMethod != null) {
                              expenseController.addExpenseRecode(
                                  _titleExpenseController.text,
                                  _Expanse_category.toString(),
                                  double.parse(
                                      _amountExpenseController.text.toString()),
                                  _expanseMethod.toString(),
                                  _dateTime,
                                  _notesExpenseController.text);
                              Navigator.pop(context);
                              showTopTitleSnackBar(context, Icons.comment_sharp,
                                  "Expense Recoded");
                            }
                          }
                        },
