...
import 'package:fem/Screens/Data_Input_Screen/family_edit_expense_transcation_screen.dart';
...
class FamilyTransaction {
  ...
  factory FamilyTransaction.fromFirestore(DocumentSnapshot doc) {
    final data = doc.data() as Map<String, dynamic>;
    var temp = decryptData(data['additional_info'], gValue.familyKey.value.key);
    return FamilyTransaction(
        id: data['uid'],
        title: decryptData(data['title'], gValue.familyKey.value.key),
        category: decryptData(data['category'], gValue.familyKey.value.key),
        date: DateTime.parse(data['date_time']),
        amount: double.parse(decryptData(data['amount'].toString(), gValue.familyKey.value.key)),
        type: decryptData(data['type'], gValue.familyKey.value.key),
        memberId: decryptData(data['memberId'], gValue.familyKey.value.key),
        memberName: decryptData(data['memberName'], gValue.familyKey.value.key),
        method: decryptData(data['method'], gValue.familyKey.value.key),
        additional: temp == " " ? "" : temp
    );
  }
}

class FamilyTransactionScreen extends StatefulWidget {
  const FamilyTransactionScreen({Key? key}) : super(key: key);

  @override
  State<FamilyTransactionScreen> createState() => _FamilyTransactionScreenState();
}

DateTime _startDate = DateTime.now().subtract(Duration(days: 7));
DateTime _endDate = DateTime.now();

class _FamilyTransactionScreenState extends State<FamilyTransactionScreen> {
  final commanValue cValue = Get.put(commanValue());
  final FamilyGroupKey gValue = Get.put(FamilyGroupKey());
  final FamilyExpenseController expenseController = Get.put(FamilyExpenseController());
  ...
  List<FamilyTransaction> _filteredTransactions = [];
  List<FamilyTransaction> _tempTransactions = [];
  List<FamilyTransaction> _exportData = [];
  bool _loading = false;
  bool _hasMore = true;
  ...
  Future<void> _fetchData() async {
    ...
    final newTransactions = querySnapshot.docs.map((doc) => FamilyTransaction.fromFirestore(doc)).toList();
    setState(() {
  ...
  void _searchTransactions(String query) async {
    ...
    try {
      ...
      final _filteredTransactions = querySnapshot.docs.map((doc) => FamilyTransaction.fromFirestore(doc)).toList();
      setState(() {
        ...
        transaction.title.toString().toLowerCase().contains(query.toLowerCase()) ||
            ...
            transaction.category.toString().toLowerCase().contains(query.toLowerCase())).toList();
        _loading = false;
      });
    } catch (e) {
      print(e);
  ...
  Future<void> exportData(DateTime Start, DateTime End) async {
    ...
    final _exportData = querySnapshot.docs.map((doc) => FamilyTransaction.fromFirestore(doc)).toList();
    ...
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: 0)).value = 'Family Transaction Records';
    ...
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 6, rowIndex: 1)).value = 'Member Name';
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 7, rowIndex: 1)).value = 'Amount';
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 8, rowIndex: 1)).value = 'Additional Information';
    ...
    for (var i = 0; i < _exportData.length; i++) {
      ...
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 6, rowIndex: i + 2)).value = _exportData[i].memberName;
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 7, rowIndex: i + 2)).value = _exportData[i].amount.toStringAsFixed(2);
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 8, rowIndex: i + 2)).value = _exportData[i].additional;
    }
    ...
    final fileName = 'Family Transaction Records From ${DateFormat("dd-MM-yy").format(Start).toString()} to ${DateFormat("dd-MM-yy").format(End).toString()}';
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    if (index == _transactions.length) {
                      ...
                      FamilyTransaction transaction = _transactions[index];
                      return Slidable(
                        ...
                        endActionPane: ActionPane(
                          ...
                          children: [
                            SlidableAction(
                              ...
                              onPressed: (context) {
                                if( _transactions[index].memberId == cValue.currentUser.value.uid){
                                  Navigator.push(context, MaterialPageRoute(builder: (context) =>
                                      FamilyEditExpenseTransactionScreen(transaction: _transactions[index])));
                                } else {
                                  showTopTitleSnackBar(context, Icons.warning_amber, "You Can't Edit This Recode");
                                }
                              },
                            ),
                            SlidableAction(
                              ...
                              onPressed: (value) async {
                                if(_transactions[index].memberId == cValue.currentUser.value.uid){
                                  await expenseController.deleteExpenseRecode(_transactions[index].id.toString());
                                  showTopTitleSnackBar(context, Icons.delete_rounded, "Recode Deleted Successfully");
                                  setState(() {
                                    _transactions.removeAt(index);
                                  });
                                } else {
                                  showTopTitleSnackBar(context, Icons.warning_amber, "You Can't Deleted This Recode");
                                }
                              },
                            )
                          ],
